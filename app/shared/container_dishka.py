...
class AppProvider(Provider):
    ...
    project_repo = provide(ProjectRepository, provides=IProjectRepository, scope=Scope.REQUEST)
    document_repo = provide(DocumentRepository, provides=IDocumentRepository, scope=Scope.REQUEST)
    redis_repo = provide(RedisRepository, provides=IRedisRepository, scope=Scope.REQUEST)
    project_invite_repo = provide(ProjectInviteRepository, provides=IProjectInviteRepository, scope=Scope.REQUEST)

    auth_service = provide(AuthService, provides=IAuthService, scope=Scope.REQUEST)
    project_service = provide(ProjectService, provides=IProjectService, scope=Scope.REQUEST)
    document_service = provide(DocumentService, provides=IDocumentService, scope=Scope.REQUEST)
