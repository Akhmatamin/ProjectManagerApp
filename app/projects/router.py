...
projects_router = APIRouter(
    prefix="/projects", tags=["projects"], route_class=DishkaRoute
)
project_router = APIRouter(prefix="/project", tags=["project"], route_class=DishkaRoute)


@projects_router.post(
    "/", response_model=ProjectCreatedSchema, status_code=status.HTTP_201_CREATED
)
...
@projects_router.get(
    "/", response_model=list[ProjectsListSchema], status_code=status.HTTP_200_OK
)
...
async def invite_member(
    ...
    return await project_service.invite_member(
        project_id, user, current_user.id, permission
    )
...
async def share_project(
    project_id: uuid.UUID,
    email: Annotated[str, Query(description="Email to share")],
    permission: Annotated[
        ProjectPermission,
        Query(
            description="Permission for the shared user",
        ),
    ],
    current_user: Annotated[User, Depends(get_current_user)],
    project_service: FromDishka[IProjectService],
    ...
    return await project_service.share_project_link(
        project_id, email, current_user.id, permission
    )


@project_router.post("/join", status_code=status.HTTP_200_OK)
async def join_project(
    token: Annotated[str, Query(description="Invite token")],
    current_user: Annotated[User, Depends(get_current_user)],
    project_service: FromDishka[IProjectService],
):
...
@project_router.post(
    "/{project_id}/documents", response_model=dict, status_code=status.HTTP_201_CREATED
)
...
@project_router.get(
    ...
    response_model=list[DocumentsListSchema],
    status_code=status.HTTP_200_OK,
)
