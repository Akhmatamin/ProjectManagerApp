...
class ProjectMember(Base):
    ...
    permission: Mapped[ProjectPermission] = mapped_column(
        SQLEnum(ProjectPermission, values_callable=lambda x: [e.value for e in x]),
        default=ProjectPermission.READ,
        nullable=False,
    )
    user: Mapped["User"] = relationship("User", back_populates="project_memberships")
    project: Mapped["Project"] = relationship(
        "Project", back_populates="user_memberships"
    )
...
class Project(Base):
    ...
    user_memberships: Mapped[list["ProjectMember"]] = relationship(
        "ProjectMember", back_populates="project", cascade="all, delete-orphan"
    )

    documents: Mapped[list["Document"]] = relationship(
        "Document", back_populates="project_document", cascade="all, delete-orphan"
    )
