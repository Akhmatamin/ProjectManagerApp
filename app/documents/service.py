...
class DocumentService(IDocumentService):
    ...
    async def _ensure_write_permission(
        self, project_id: uuid.UUID, current_user_id: uuid.UUID
    ):
        permission = await self.project_repo.get_user_permission(
    ...
    async def upload_document(
        self, new_file, project_id: uuid.UUID, current_user_id: uuid.UUID
    ):
    ...
    async def get_all_project_documents(
        self, project_id: uuid.UUID, current_user_id: uuid.UUID
    ):
        project = await self.project_repo.get_if_user_member(
            project_id, current_user_id
        )
        if not project:
            raise NotMemberOrNoProject()
    ...
    async def download_document(
        ...
        project = await self.project_repo.get_if_user_member(
            document.project_id, current_user_id
        )
        if not project:
            raise NotMemberOrNoProject()
    ...
    async def update_document(
        self, new_file: UploadFile, document_id: uuid.UUID, current_user_id: uuid.UUID
    ):
        document = await self.document_repo.get_by_id(document_id)
        if not document:
            raise DocumentNotFound()
