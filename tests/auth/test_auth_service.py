...
class TestRegisterUser:
    async def test_register_user_success(self, service, user_repo):
        ...
        with patch(
            "app.auth.service.get_password_hash",
            new=AsyncMock(return_value="new-password"),
        ):
...
class TestLogin:
    async def test_login_success(self, service, user_repo, sample_user):
        ...
        with (
            patch("app.auth.service.verify_password", new=AsyncMock(return_value=True)),
            patch(
                "app.auth.service.create_access_token",
                new=AsyncMock(return_value="access"),
            ),
            patch(
                "app.auth.service.create_refresh_token",
        ...
        user_repo.save_or_update_token.assert_awaited_once_with(
            sample_user.id, "refresh"
        )
    ...
    async def test_login_wrong_password(self, service, user_repo, sample_user):
        ...
        with (
            patch(
                "app.auth.service.verify_password", new=AsyncMock(return_value=False)
            ),
...
class TestRefreshNewToken:
    async def test_refresh_new_token_success(self, service, user_repo):
        ...
        with (
            patch("app.auth.service.token_expired", return_value=False),
            patch(
                "app.auth.service.create_access_token",
                new=AsyncMock(return_value="access"),
            ),
...
class TestChangePassword:
    async def test_change_password_success(self, service, user_repo, sample_user):
        ...
        with (
            ...
            patch(
                "app.auth.service.get_password_hash",
                new=AsyncMock(return_value="new-pass"),
            ),
        ):
    ...
    async def test_change_password_invalid_old_password(
        self, service, user_repo, sample_user
    ):
        data = ChangePasswordSchema(
            old_password="wrong-old-pass", new_password="new-pass"
        )
...
class TestRequestResetCode:
    async def test_request_reset_code_success(
        self, service, user_repo, redis_repo, email_service, settings, sample_user
    ):
        user_repo.get_user_by_email.return_value = sample_user
        ...
        email_service.send_reset_code.assert_awaited_once_with(
            sample_user.email, "1212"
        )
        redis_repo.save_code.assert_awaited_once_with(
            sample_user.email,
            "1212",
    ...
    async def test_request_reset_code_wrong_email(
        self, service, user_repo, email_service
    ):
        user_repo.get_user_by_email.return_value = None
...
class TestResetPassword:
    async def test_reset_password_success(
        self, service, user_repo, redis_repo, sample_user
    ):
        ...
        with patch(
            "app.auth.service.get_password_hash", new=AsyncMock(return_value="new-pass")
        ):
